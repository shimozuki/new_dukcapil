...
class DocumentRepository extends BaseRepository
{
    ...
    public function getDocument($search=null,$tag=[],$status=null,$create_by=null, $paginate=true)
    {
        $query = $this->allQuery($search);
        if(!empty($tag)){
            $query->whereHas('tags', function ($q) use ($tag) {
        ...
        if(!empty($status)){
            $query->where('status',$status)->where('created_by', $create_by);
        }
        $query = $query->where('created_by', $create_by)->with('tags');
        debug($query->toSql());
        if($paginate)
            return $query->paginate(25);
        else
    ...
    public function rejectDoc($document, $pesan)
    {
        $document->status = config('constants.STATUS.REJECT');
        $document->pesan = $pesan;
        $document->save();
    }
